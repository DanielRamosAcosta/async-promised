...
describe("detect", () => {
  ...
  it("detect", async () => {
    const callOrder = [];
    await async
      .detect([3, 2, 1], createCallOrderDetector(callOrder))
      .catch(err => {
  ...
  it("detect - mulitple matches", async () => {
    const callOrder = [];
    await async
      .detect([3, 2, 2, 1, 2], createCallOrderDetector(callOrder))
      .catch(err => {
        expect(err).toEqual(null);
      })
      .then(result => {
  ...
  it("detectSeries", async () => {
    ...
    await async
      .detectSeries([3, 2, 1], createCallOrderDetector(callOrder))
      .catch(err => {
        expect(err).toEqual(null);
      })
      .then(result => {
  ...
  it("detectSeries - multiple matches", async () => {
    ...
    await async
      .detectSeries([3, 2, 2, 1, 2], createCallOrderDetector(callOrder))
      .catch(err => {
        expect(err).toEqual(null);
      })
      .then(result => {
  ...
  it("detectSeries - ensure stop", () => {
    return async
      .detectSeries([1, 2, 3, 4, 5], async num => {
        if (num > 3) {
  ...
  it("detectLimit", async () => {
    ...
    await async
      .detectLimit([3, 2, 1], 2, createCallOrderDetector(callOrder))
      .catch(err => {
        expect(err).toEqual(null);
      })
      .then(result => {
  ...
  it("detectLimit - multiple matches", async () => {
    ...
    await async
      .detectLimit([3, 2, 2, 1, 2], 2, createCallOrderDetector(callOrder))
      .catch(err => {
        expect(err).toEqual(null);
      })
      .then(result => {
  ...
  it("detectLimit - ensure stop", () => {
    return async
      .detectLimit([3, 2, 2, 1, 2], 2, async num => {
        if (num > 4) {
  ...
  it("detectLimit doesn't cause stack overflow (#1293)", async () => {
    ...
    return async
      .detectLimit(arr, 100, async data => {
        calls += 1;
        await async.setImmediate();
        return true;
      })
